...
pub(crate) fn get_text_size(font_db: &fontdb::Database, text: &StyledText) -> (f32, f32) {
    ...
    if let Some(child) = tree.root.first_child() {
        ...
        for line in &text.styled_lines {
            width = width.max((0..line.spans.len()).map(|_| {
...
fn render_line(text_styles: &[TextStyle], styled_line: &StyledLine, x: f32, y: f32) -> TextChunk {
    TextChunk {
        ...
        spans: styled_line.spans.iter().map(|span| create_svg_span(text_styles, span)).collect(),
...
pub(crate) fn render_text(styled_text: &StyledText, x: f32, y: f32) -> usvg::Node {
    let n_chars = styled_text.styled_lines.iter().map(|sl| sl.text.len()).sum();
    ...
    let mut current_y = y;
    let chunks: Vec<TextChunk> = styled_text.styled_lines.iter().map(|styled_line| {
        let line_height = styled_line.line_height(&styled_text.styles).unwrap_or_else(|| styled_text.default_line_height());
        let font_size = styled_line.font_size(&styled_text.styles).unwrap_or(styled_text.default_font_size);
