use crate::Page;
use crate::render::composer::{Composer, SvgWriteComposer};
...
impl Document {
    ...
    pub fn render_svg_to_dir(&self, resources: &Resources, path: &std::path::Path) -> crate::Result<()> {
        let composer = SvgWriteComposer::new(path, self.pages.len());
        self.render(resources, &composer)
    }

    pub fn render_png_to_dir(&self, resources: &Resources, path: &std::path::Path) -> crate::Result<()> {
        todo!()
        /*let composer = todo!();
        self.render(resources, &composer)*/
    }
