...
use std::sync::Mutex;
use crate::{NodeId, Rectangle, Resources};
...
impl LayoutInfoComposer {
    pub fn new(n_pages: usize) -> Self {
        Self {
            page_layouts: Mutex::new(vec![PageLayout { node_layouts: HashMap::new() }; n_pages]),
        }
    }
...
impl Composer for LayoutInfoComposer {
    fn add_page(&self, page_idx: usize, _canvas: Canvas, _content_map: &ContentMap, compute_layout: &ComputedLayout) -> crate::Result<()> {
        let node_layouts = compute_layout.iter().map(|(node_id, data)| {
            (*node_id, data.rect.clone())
        }).collect();
